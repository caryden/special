"""Tests for whenwords — all test vectors from SPEC.md."""
...
class TestTimeAgoPast:
    ...
    def test_past(self, timestamp, expected):
...
class TestTimeAgoFuture:
    ...
    def test_future(self, timestamp, expected):
        assert time_ago(timestamp, REF_TIME_AGO) == expected
...
class TestDuration:
    ...
    def test_verbose(self, seconds, expected):
    ...
    def test_compact(self, seconds, expected):
        assert duration(seconds, compact=True) == expected

    @pytest.mark.parametrize("seconds, max_u, expected", [
        (3661, 1, "1 hour"),
        (93600, 1, "1 day"),
        (93661, 3, "1 day, 2 hours, 1 minute"),
    ])
    def test_max_units(self, seconds, max_u, expected):
...
class TestParseDuration:
    ...
    def test_errors(self, input_str):
        with pytest.raises(ValueError):
...
REF_HUMAN_DATE = 1705276800  # 2024-01-15 Monday 00:00 UTC
...
class TestHumanDate:
    ...
    def test_human_date(self, timestamp, expected):
...
class TestDateRange:
    ...
    def test_date_range(self, start, end, expected):
